...
Visible repair verification kept the prior scene while the color function was broken, queued a click, and applied it exactly once after a one-form repair. The native suite has 74 passing tests, all five protocol scripts pass, and three Emacs integration tests pass.
...
This implementation is in progress. Every ordinary struct submitted to the live program is versioned automatically; persistent roots, recursive embedded-record evolution, explicit field renames, typed field transitions and transition-only deferred repair work without a struct annotation. Owned `ArrayList` buffers of plain values migrate transactionally, and borrowed slices into those buffers retain their aliases. See [schema contracts](docs/SCHEMAS.md). Element-wise migration for resource-bearing or schema-changing container elements, output streaming, watcher integration, automated pixel comparison and broader retention/failure gates remain. Direct semantic repair and input cancellation work; unanalysed calls use conservative admission, and expressions are rejected while any function is blocked. Generic, attributed, direct `defn*`, sum, `const`, and ordinary `def` definitions now update automatically with retained dependent regeneration. Arbitrary native-stack continuation repair is outside the design.
